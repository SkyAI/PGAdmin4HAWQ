...
class DatabaseView(PGChildNodeView):
    ...
    def list(self, gid, sid):
        ...
        servertype = self.manager.server_type
        if 'HAWQ' in self.manager.ver:
            servertype = 'hawq'
        SQL = render_template(
    ...
    def properties(self, gid, sid, did):
        ...
        SQL = render_template(
            "/".join([self.template_path, 'properties.sql']),
            did=did, conn=self.conn, last_system_oid=0, servertype=servertype
        )
        ...
        SQL = render_template(
            ...
            did=did, conn=self.conn, servertype=servertype
        )
    ...
    def create(self, gid, sid):
        ...
        SQL = render_template(
            "/".join([self.template_path, 'properties.sql']),
            name=data['name'], conn=self.conn, last_system_oid=0, servertype=servertype
        )
        SQL = SQL.strip('\n').strip(' ')
        if SQL and SQL != "":
            status, res = self.conn.execute_dict(SQL)
    ...
    def statistics(self, gid, sid, did=None):
        ...
        status, res = conn.execute_dict(render_template(
            ...
            conn=conn,
            last_system_oid=last_system_oid,
            db_restrictions=db_disp_res, servertype=servertype),
            params
        )
    ...
    def sql(self, gid, sid, did):
        ...
        SQL = render_template(
            "/".join([self.template_path, 'properties.sql']),
            did=did, conn=conn, last_system_oid=0, servertype=servertype
        )
