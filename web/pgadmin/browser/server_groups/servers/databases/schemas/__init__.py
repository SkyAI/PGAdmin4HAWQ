    ...
    def wrap(*args, **kwargs):
        ...
        if self.manager.server_type == 'gpdb':
            if 'HAWQ' in self.manager.ver:
                _temp = self.hawq_template_path(self.manager.version)
            else:
...
class SchemaView(PGChildNodeView):
    ...
    * dependent(gid, sid, did, scid):
    ...
    @staticmethod
    def hawq_template_path(ver):
        """
        Returns the template path for hawq servers.
        """
        return '#hawq#{0}#'.format(ver)
...
"""))
        ...
        returnnodes = []
        if 'HAWQ' in self.manager.ver:
            exceptnodes = ['pgadmin.node.fts_template',
            ...
            for node in nodes:
                if node['module'] not in exceptnodes:
                    returnnodes.append(node)
        else:
            returnnodes = nodes
        return make_json_response(data=returnnodes)
