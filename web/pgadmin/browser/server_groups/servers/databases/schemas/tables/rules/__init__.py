...
class RuleView(PGChildNodeView):
    ...
    * operations - function routes mappings defined.
    ...
    def list(self, gid, sid, did, scid, tid):
        ...
        SQL = render_template("/".join(
            [self.template_path, 'properties.sql']), tid=tid, servertype=servertype)
    ...
    def properties(self, gid, sid, did, scid, tid, rid):
        ...
        ), rid=rid, datlastsysoid=self.datlastsysoid, servertype=servertype)
        status, res = self.conn.execute_dict(SQL)
    ...
    def sql(self, gid, sid, did, scid, tid, rid):
        ...
        servertype = 'hawq' if 'HAWQ' in self.manager.ver else self.manager.server_type
        SQL = render_template("/".join(
            [self.template_path, 'properties.sql']), rid=rid, servertype=servertype)
        status, res = self.conn.execute_dict(SQL)
        if not status:
            return internal_server_error(errormsg=res)
        if len(res['rows']) == 0:
