SELECT *,
	(CASE when pre_coll_inherits is NULL then NULL::varchar[] else pre_coll_inherits END) as coll_inherits
...
-- 		substring(array_to_string(rel.reloptions, ',') FROM 'appendonly=(true|false)')::boolean AS appendonly,
		(CASE WHEN (substring(array_to_string(rel.reloptions, ',') FROM 'appendonly=(true|false)') = 'true') THEN true else false end) AS appendonly,
		...
		(CASE WHEN rel.reltoastrelid = 0 THEN false ELSE true END) AS hastoasttable,
			-- Added for pgAdmin4
    (CASE WHEN (substring(array_to_string(rel.reloptions, ',') FROM 'autovacuum_enabled=([a-z|0-9]*)')) = 'true'  THEN true ELSE false END) AS autovacuum_custom,
 		(CASE WHEN (substring(array_to_string(tst.reloptions, ',') FROM 'autovacuum_enabled=([a-z|0-9]*)')) = 'true'  AND rel.reltoastrelid != 0 THEN true ELSE false END) AS toast_autovacuum,

		NULL::varchar[] AS seclabels,
