...
class ServerNode(PGChildNodeView):
    ...
    def statistics(self, gid, sid):
        ...
        if conn.connected():
            servertype = 'hawq' if 'HAWQ' in manager.ver else manager.server_type
            temp_path = "/servers/sql/hawq/stats.sql" if servertype == 'hawq' \
                else "/servers/sql/#{0}#/stats.sql".format(manager.version)
            status, res = conn.execute_dict(
                render_template(
                    temp_path, conn=conn, _=gettext
                )
            )
